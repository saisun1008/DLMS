...
public class InternalRequestUDPListener implements Runnable
{
			...
			try
			{
				...
				if (answer != null)
				{
					...
					if (answer.getType() == messageType.TransferAnswer)
					{
						...
						{
							System.err
									.println("Loan already exists on bank server "
					...
					if (answer.getType() == messageType.TransferAnswer)
					{
						// if nothing wrong happened during the reply phase,
						// then we can add the
	...
	private LoanProtocol processProtocol(LoanProtocol protocol)
	{
		...
		case LoanAnswer:
			...
			return null;
		case ValidateAdmin:
			boolean result = m_server.validateAdminUser(protocol.getUser()
					.getUsr(), protocol.getUser().getPassword());
			LoanProtocol l = new LoanProtocol("", protocol.getHost(),
					protocol.getPort(), null, null);
			l.setResult(result);
			return l;
		case Transfer:
	 ...
	 * @param protocol contains info to be rolled back
	 * @return
	 */
	...
	private LoanProtocol generateAnswer(LoanProtocol request)
	{
		...
		LoanProtocol answer = new LoanProtocol(request.getId(),
				request.getHost(), request.getPort(), user,
				messageType.LoanAnswer);
		return answer;
	}
